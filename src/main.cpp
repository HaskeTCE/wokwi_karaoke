...
const int DOOR_CLOSED_THRESHOLD = 10; // cm - adjust based on closed door distance

bool intendToLock = true;
...
void setup()
{
    ...
    lockServo.write(90); // Physically start unlocked
    isCurrentlyLocked = false;
    intendToLock = true; // Locked state
...
void loop()
{
    manageDoorLock(); // Ensure door lock state is consistent with user intent
    checkPowerSwitch();
...
float getDoorDistance()
{
    ...
    // 15000us timeout to prevent loop lag when door is open (~2.5 meters max range)
    long duration = pulseIn(ECHO_PIN, HIGH, 15000);

    if (duration == 0)
        return 999; // No echo received, door is wide open
...
void manageDoorLock()
{
    if (intendToLock)
    {
        // Only check sensor if door is currently unlocked
        if (!isCurrentlyLocked)
        {
            if (getDoorDistance() <= DOOR_CLOSED_THRESHOLD)
            {
                lockServo.write(0); // Physically lock it
    ...
    {
        // If intend to unlock, do it immediately
        if (isCurrentlyLocked)
        {
            lockServo.write(90);
            isCurrentlyLocked = false;
